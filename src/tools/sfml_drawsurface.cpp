...
void sfml_drawsurface::circleColor(Point& a, unsigned int radius, std::string color) {
	...
	circle.setPosition(sf::Vector2f(a.get_x()-radius/2.0, a.get_y()-radius/2.0));
...
void sfml_drawsurface::trigonColor(Point& a, unsigned int _size, std::string color) {
	Point a1(a.get_x(), a.get_y()-_size);
	Point a2(a.get_x()-_size, a.get_y()+_size);
	Point a3(a.get_x()+_size, a.get_y()+_size);

	trigonColor(a1, a2, a3, color);
}

int sfml_drawsurface::get_fontsize() {
